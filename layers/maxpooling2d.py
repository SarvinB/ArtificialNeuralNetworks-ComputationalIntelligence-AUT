...
class MaxPool2D:
    ...
    def target_shape(self, input_shape):
        ...
        H = int((input_shape[1]-self.kernel_size[0])/self.stride[0])
        W = int((input_shape[2]-self.kernel_size[1])/self.stride[1])
        return H, W
    ...
    def forward(self, A_prev):
        ...
        for i in range(batch_size):
            for h in range(0, H, strideh):
                h_start = h
                h_end = h_start + f_h
                for w in range(0, W, stridew):
                    w_start = w
                    w_end = w_start + f_w
    ...
    def backward(self, dZ, A_prev):
        ...
        strideh, stridew = self.stride
        batch_size, H_prev, W_prev, C_prev = A_prev.shape
        batch_size, (H, W), C = A_prev.shape[0], self.target_shape(A_prev.shape), C_prev
                ...
                for w in range(0, W, stridew):
                    for c in range(C):
                        ...
                        if self.mode == "max":
                            ...
                            dA_prev[i, h_start:h_end, w_start:w_end, c] += np.multiply(dZ[i, h, w, c], mask)
                        elif self.mode == "average":
                            dz = dZ[i, h, w, c]
